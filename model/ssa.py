...
class ShuntedTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            if i == 0:
                ...
                patch_embed = OverlapPatchEmbed(img_size=img_size // (2 ** 4) if i == 4 else img_size // (2 ** (i + 1)),
                                                patch_size=3 if i == 4 else 3,
                                                stride=1 if i == 4 else 2,
                                                in_chans=embed_dims[i - 1] if i == 4 else embed_dims[i - 1],
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            out_block = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            out_blocks.append(out_block)

            if i != self.num_stages - 1:
                x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        return x.mean(dim=1), out_blocks

    def forward(self, x):
        x, blocks = self.forward_features(x)
        x = self.head(x)
...
def shunted_t(pretrained=False, **kwargs):
    ...
    if os.path.exists(path):
        ...
        print('*' * 20, 'Load SSA Pretrained Model Done ! ! !', '*' * 20)
